...
import { createKaryawan, getJabatan, getShift } from '../api' // ← TAMBAH: Ambil getShift dari API
...
export default function TambahKaryawan() {
  ...
  const [shiftList, setShiftList] = useState([]) // Kish ← TAMBAH: State untuk menampung shift dari database
  ...
  useEffect(() => {
    // Ambil data Jabatan dan Shift secara bersamaan saat halaman dibuka
    Promise.all([getJabatan(), getShift()])
      .then(([jabRes, shiftRes]) => {
        ...
        // Pasang nilai default pertama kali jika data tersedia
        setForm(f => ({
          ...f, 
          jabatan_id: jabRes.data.length ? jabRes.data[0].id : '',
          shift_id: listShift.length ? listShift[0].id : '' // Otomatis pilih shift pertama
        }))
      })
    ...
    <div style={{paddingBottom:24}}>
      ...
      <div style={{padding:'16px 20px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
          ...
          {/* ── SEKARANG SUDAH SINKRON TOTAL DENGAN DATABASE ── */}
          <div>
            <div style={{fontSize:11,color:'var(--text3)',fontWeight:500}}>Shift *</div>
            <select {...inp('shift_id')}>
              <option value="">Pilih</option>
              {shiftList.map(s => (
        ...
        <div style={{marginBottom:20}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:6}}>
            {DAYS.map((d,i) => {
              const v=jadwal[i]; const c=shiftCfg[v]
