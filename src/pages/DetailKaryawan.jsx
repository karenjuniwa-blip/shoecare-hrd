...
export default function DetailKaryawan() {
  ...
  // Ambil tanggal awal dan akhir bulan ini untuk default filter tanggal kustom
  const yyyy = now.getFullYear()
  const mm = String(now.getMonth() + 1).padStart(2, '0')
  const lastDay = new Date(yyyy, now.getMonth() + 1, 0).getDate()
  
  const defaultStartDate = `${yyyy}-${mm}-01`
  const defaultEndDate   = `${yyyy}-${mm}-${String(lastDay).padStart(2, '0')}`
  ...
  // State untuk rekap absen bulanan
  const [bulanRekap, setBulanRekap]     = useState(bulan)
  const [tahunRekap, setTahunRekap]     = useState(tahun)
  const [loadingRekap, setLoadingRekap] = useState(false)
  const [rekapData, setRekapData]       = useState(null)

  // STATE BARU: Filter Tanggal Slip Gaji Kustom
  const [tglMulai, setTglMulai]     = useState(defaultStartDate)
  const [tglSelesai, setTglSelesai] = useState(defaultEndDate)
  ...
  // FUNGSI BARU: Filter Slip Gaji menggunakan parameter tanggal kustom
  async function tanganiFilterGaji() {
    setLoadingGaji(true)
    try {
      const gRes = await getGaji(id, null, null, { dari: tglMulai, sampai: tglSelesai })
      setGaji(gRes.data)
      alert(`Berhasil memfilter slip gaji periode ${tglMulai} s/d ${tglSelesai}`)
    } catch (error) {
      console.error("Gagal memfilter gaji:", error)
      alert("Gagal memfilter slip gaji. Periksa kembali integrasi API.")
    } finally {
  ...
  function getBatasJamMasukByTanggal(tglStr) {
    ...
    const dateOnly = tglStr.split('T')[0]
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px 12px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg)', zIndex: 50 }}>
        ...
        <div style={{ fontSize: 11, background: 'var(--accentbg,#1e2d4a)', color: 'var(--accent)', padding: '3px 10px', borderRadius: 20 }}>
          Admin
        </div>
      </div>
      ...
      {tab === 'ringkasan' && (
        <>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, margin: '0 16px 10px' }}>
            ...
            <div style={{ background: 'var(--bg3)', borderRadius: 'var(--radius-sm)', padding: '10px 12px', marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: telat > 0 ? 'var(--red)' : 'var(--green)', fontFamily: 'DM Mono,monospace' }}>
                  {telat}×
                </span>
              </div>
              {telat > 0 && (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: hadirPct >= 85 ? 'var(--green)' : hadirPct >= 70 ? 'var(--amber)' : 'var(--red)' }}>
                {hadirPct}%
              </div>
            </div>
          ...
          {/* ── SEKSI BARU: FILTER TANGGAL SLIP GAJI KUSTOM ── */}
          <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.08em', padding: '14px 20px 6px' }}>
            Filter Periode Slip Gaji
          </p>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, margin: '0 16px 10px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <label style={{ fontSize: 11, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>Dari Tanggal:</label>
                <input 
                  type="date" 
                  value={tglMulai} 
                  onChange={e => setTglMulai(e.target.value)} 
                  style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', color: 'var(--text)', fontSize: 13, padding: '8px 10px', width: '100%', fontFamily: 'inherit', outline: 'none' }}
                />
              </div>
              <div>
                <label style={{ fontSize: 11, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>Sampai Tanggal:</label>
                <input 
                  type="date" 
                  value={tglSelesai} 
            ...
            <button 
              onClick={tanganiFilterGaji} 
              disabled={loadingGaji}
              style={{ width: '100%', padding: 10, background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'opacity .2s' }}
            >
              {loadingGaji ? 'Memproses...' : '🔍 Terapkan Filter Periode'}
            </button>
          </div>
            ...
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, margin: '0 16px 10px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, background: 'var(--accentbg,#1e2d4a)', borderRadius: 'var(--radius-sm)', padding: 13, border: '1px solid var(--accent2,#1d4ed8)' }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'DM Mono,monospace', color: 'var(--accent)' }}>
                  {rupiah(totalGajiBersihAkhir > 0 ? totalGajiBersihAkhir : 0)}
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, margin: '0 16px 10px' }}>
            {[
              ...
              <div key={f.l}>
                <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 10 }}>{f.l}</div>
                {f.el}
              </div>
            ))}
              ...
              {HARI.map((h, i) => {
                ...
                return (
                  <div key={h} style={{ textAlign: 'center' }}>
                    ...
                    <button onClick={() => cycleJadwal(i)} style={{ width: 34, height: 34, borderRadius: 8, border: `1.5px solid ${buttonCfg.bc}`, background: buttonCfg.bg, color: buttonCfg.c, cursor: 'pointer', fontSize: 9, fontWeight: 700, fontFamily: 'inherit' }}>
                      {buttonCfg.l}
                    </button>
                  </div>
                )
      ...
      {/* Konten Tab Rekap Absen */}
      {tab === 'rekap' && (
        <>
          <div style={{ padding: '14px 16px 8px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.06em', flexShrink: 0 }}>
              Periode
            </span>
            <select
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 1fr 60px', gap: 6, padding: '4px 16px 6px', borderBottom: '1px solid var(--border)' }}>
            {['Tanggal', 'Masuk', 'Keluar', 'Status'].map(h => (
              <div key={h} style={{ fontSize: 10, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.05em' }}>
                {h}
              </div>
            ))}
          </div>
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, fontFamily: 'DM Mono,monospace', color: d.jam_masuk ? (isTelat ? 'var(--red)' : 'var(--green)') : 'var(--text3)' }}>
                    {d.jam_masuk || '—'}
                  </span>
                  {d.status === 'hadir' && (
                ...
                <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'DM Mono,monospace', color: d.jam_keluar ? 'var(--text2)' : 'var(--text3)' }}>
                  {d.jam_keluar || '—'}
                </div>

                <div style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 20, background: ss.bg, color: ss.color, textAlign: 'center', whiteSpace: 'nowrap' }}>
                  {ss.label}
                </div>
              </div>
            )
            ...
            <div style={{ margin: '12px 16px 24px', background: 'var(--redbg)', border: '1px solid var(--red)', borderRadius: 'var(--radius)', padding: '12px 14px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                ...
                <div style={{ background: 'rgba(239,68,68,0.1)', borderRadius: 'var(--radius-sm)', padding: '8px 10px' }}>
                  ...
                  <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--red)', fontFamily: 'DM Mono,monospace' }}>
                    {Math.round(totalMenitTelat / telat)} mnt
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ margin: '12px 16px 24px', background: 'var(--greenbg)', border: '1px solid var(--greenborder,#1a4429)', borderRadius: 'var(--radius)', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                  {BULAN_ID[bulanRekap - 1]} {tahunRekap} — disiplin penuh
                </div>
              </div>
            </div>
          )}
